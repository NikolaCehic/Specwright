...
- README source-checkout CLI guidance includes `doctor` and now reflects that `approve` and `reject` route through `RuntimeApi.recordApproval` with fail-closed stale/missing approval behavior.
...
- CLI usage exposes `doctor`, `run`, `status`, `events`, `replay`, `report`, `approve`, `reject`, and `answer`.
